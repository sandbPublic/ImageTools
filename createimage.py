...
def create_spectra():
    ...
    im = im.convert('HSV')
    im = colormanip.map_pixels(im, lambda c: colormanip.box_swap_hsv(c,
                                                                     [100, 156, 0, 255, 0, 255],
                                                                     [95, 161, -1, 256, -1, 256]),
                               [0, 255, 64, 255])
    im = im.convert('RGB')
...
def create_composite_mask(im, mask, byte_source_box: List[int],
    ...
    for x in range(mask.size[0]):
        for y in range(mask.size[1]):
            ...
            image_hsv[0] = colormanip.hue_nearest_bands(image_hsv[0], byte_source_box[0], byte_source_box[1])
        ...
        for y in range(bounding_box[2], bounding_box[3]):
            if mask_array[x][y]:
                ...
                if hsv[1] < sat_min or hsv[2] < val_min:
                    new_hue = colormanip.hue_nearest_bands(hsv[0], hue_min, hue_max)
...
def create_color_permutations():
    ...
    im = colormanip.linear_map_colors(im, [[1, 0, 0], [0, 0, 1], [0, 1, 0]])
    im.save(FILE_PREFIX + '_rbg.png')

    im = Image.open(FILE_PREFIX + '.png')
    im = colormanip.linear_map_colors(im, [[0, 1, 0], [1, 0, 0], [0, 0, 1]])
    im.save(FILE_PREFIX + '_grb.png')

    im = Image.open(FILE_PREFIX + '.png')
    im = colormanip.linear_map_colors(im, [[0, 0, 1], [1, 0, 0], [0, 1, 0]])
    im.save(FILE_PREFIX + '_brg.png')

    im = Image.open(FILE_PREFIX + '.png')
    im = colormanip.linear_map_colors(im, [[0, 1, 0], [0, 0, 1], [1, 0, 0]])
    im.save(FILE_PREFIX + '_gbr.png')

    im = Image.open(FILE_PREFIX + '.png')
    im = colormanip.linear_map_colors(im, [[0, 0, 1], [0, 1, 0], [1, 0, 0]])
...
def run():
    ...
    byte_source_box = colormanip.convert_hsv_box_to_bytes(source_box)
    ...
    for target_box, label in zip(target_boxes, box_labels):
        ...
        byte_target_box = colormanip.convert_hsv_box_to_bytes(target_box)
        ...
        for b in range(256):
            for i in range(3):
                ...
                scale = colormanip.weighted_average_factor(b, byte_source_box[2 * i], byte_source_box[2 * i + 1])
                # scale to absolute coordinates using target box
                converted = int(
                    colormanip.linear_combination(byte_target_box[2 * i], byte_target_box[2 * i + 1], scale)) % 256
